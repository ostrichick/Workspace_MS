...
<form id="frm" action="${pageContext.request.contextPath}/update_process.do" method="post">
  <div class="row g-2">
    ...
    <div class="col-6">
      <div class="p-2 border bg-light"><input type="text" name="member_name" value="${memberVo.member_name}" /></div>
    </div>
    <div class="col-6">
      <div class="p-2 border bg-light" style="height:48px;">아이디(이메일)</div>
    </div>
    <div class="col-6">
      <div class="p-2 border bg-light" style="height:48px;"><input type="hidden" name="member_id" value="${member_id}"/>${member_id}</div>
    </div>
    <div class="col-6">
      <div class="p-2 border bg-light" style="height:48px;">비밀번호</div>
    </div>
    <div class="col-6">
      <div class="p-2 border bg-light"><input type="text" name="member_pw" value="${memberVo.member_pw}" /></div>
    </div>
    <div class="col-6">
      <div class="p-2 border bg-light" style="height:48px;">핸드폰번호</div>
    </div>
    <div class="col-6">
      <div class="p-2 border bg-light"><input type="text" name="member_handphone" value="${memberVo.member_handphone}" /></div>
    </div>
  </div>
  
  <div style="margin:15px;">
	  <button type="submit" class="btn btn-primary">수정하기</button>&nbsp;&nbsp;
	  <button id="main_btn" class="btn btn-primary">메인화면</button>
  </div>
</form>  
...
<script>
	$(function(){//jQuery구문
		...
		$("#main_btn").on('click',function(event){
			event.preventDefault(); //폼태그가 전송되는 기본 이벤트 막기
