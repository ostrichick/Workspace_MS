  ...
  public void getScoreGrade(Student student, int subjectId) {
    ...
    for (int i = 0; i < scoreList.size(); i++) { // 학생이 가진 점수들
      ...
      if (score.getSubject().getSubjectId() == subjectId) { // 현재 학점을 산출할 과목
        String grade;
        if (score.getSubject().getSubjectId() == majorId) grade =
          gradeEvaluation[Define.SAB_TYPE].getGrade(score.getPoint()); // 중점 과목인 경우 //중점 과목 학점 평가 방법
        else grade = gradeEvaluation[Define.AB_TYPE].getGrade(score.getPoint()); // 중점 과목이 아닌 경우
